...
void FFT_1D(unsigned *input_real , unsigned *input_imag , unsigned *output_real , unsigned *output_imag , int length)
{
    ...
    SFP q[4] = {SFP(EXP,MANT) , SFP(EXP,MANT) , SFP(EXP,MANT) , SFP(EXP,MANT)};
    SFP o[4] = {SFP(EXP,MANT) , SFP(EXP,MANT) , SFP(EXP,MANT) , SFP(EXP,MANT)};
    if(length == 2) {
        for (int i = 0; i <length/2 ; i++) {
            q[0] = (p[0].setBits(input_real[i]) + p[1].setBits(input_real[i+length/2])).resizeTo(EXP,MANT);  //第一列real
            q[1] = (p[2].setBits(input_real[i]) - p[3].setBits(input_real[i+length/2])).resizeTo(EXP,MANT);  //第二列real
            q[2] = (p[0].setBits(input_imag[i]) + p[1].setBits(input_imag[i+length/2])).resizeTo(EXP,MANT);  //第一列imag
            q[3] = (p[2].setBits(input_imag[i]) - p[3].setBits(input_imag[i+length/2])).resizeTo(EXP,MANT);  //第二列imag
        ...
        for (int i = 0; i <length/2 ; i++) {
            ...
            p[0] = ((q[0] * o[0].set(FN1024_REAL[i*1024/2*4/length])).resizeTo(EXP,MANT) - (q[2] * o[1].set(FN1024_IMAG[i*1024/2*4/length])).resizeTo(EXP,MANT)).resizeTo(EXP,MANT);//第一列real
            // cout<<i<<"  "<<i*1024*2/length<<endl;
            // o[0].print();
            // o[1].print();
            p[1] = ((q[0] * o[2].set(FN1024_IMAG[i*1024/2*4/length])).resizeTo(EXP,MANT) + (q[2] * o[3].set(FN1024_REAL[i*1024/2*4/length])).resizeTo(EXP,MANT)).resizeTo(EXP,MANT);//第一列imag
            p[2] = ((q[1] * o[0].set(FN1024_REAL[i*1024/2*4/length+2])).resizeTo(EXP,MANT) - (q[3] * o[1].set(FN1024_IMAG[i*1024/2*4/length+2])).resizeTo(EXP,MANT)).resizeTo(EXP,MANT);//第二列real
            // o[0].print();
            p[3] = ((q[1] * o[2].set(FN1024_IMAG[i*1024/2*4/length+2])).resizeTo(EXP,MANT) + (q[3] * o[3].set(FN1024_REAL[i*1024/2*4/length+2])).resizeTo(EXP,MANT)).resizeTo(EXP,MANT);//第二列imag
